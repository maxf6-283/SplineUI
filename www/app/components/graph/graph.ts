...
export class Graph implements AfterViewInit{
  ...
  private updateCanvas() {
    if(this.data != null) {
      ...
      let subDivision = 2;
      let powerOf10 = -2; //assuming that we will never have a path that is less than 1 sec long
      while(maxtime/(subDivision*10**powerOf10) > maxTickMarks) {
        if(subDivision == 1) {
          subDivision = 2;
      ...
      let timeDivision = subDivision * 10 ** powerOf10;
      let currTime = 0;
      this.ctx.strokeStyle = "black";
      ...
      while(currTime < maxtime) {
        ...
        this.ctx.stroke();
        this.ctx.fillText(currTime.toFixed(-powerOf10), x, this.canvas.height - axisoffset + 5)
